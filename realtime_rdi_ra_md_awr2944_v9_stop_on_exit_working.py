"""
realtime_rdi_ra_md_awr2944_v6_fast.py
=====================================

Goal:
- Restore / improve FPS after adding Micro-Doppler
- Make Micro-Doppler *synchronous* with RD (no visible lag)

Key changes vs v5:
1) Micro-Doppler is computed from the *same RD result* (no extra FFT pass).
2) Micro-Doppler update rate is decoupled from RA (MD every frame, RA throttled).
3) Lighter smoothing on Micro-Doppler (separate EMA + smaller blur).
4) Optional further speed knobs exposed at the top.
"""
