"""
realtime_rdi_ra_md_awr2944_v4_1.py
=====================================
1) Sharper / More Reactive Micro-Doppler (MD)
------------------------------------------------
Moving from 1TX to 4TX (TDM-MIMO) increases the number of
virtual channels from 4 → 16 (4 TX × 4 RX). After Doppler FFT,
energy is summed across all virtual antennas, resulting in:
  • Higher effective SNR
  • Stronger moving-target signatures
  • Cleaner Doppler ridges
  • More responsive micro-Doppler plots
This is NOT increased Doppler resolution — it is improved
signal strength due to multi-TX coherent energy combining.
2) Dynamic Background / Colormap Behavior
------------------------------------------------
Range-Doppler and Micro-Doppler displays use percentile-based
auto-level scaling (AUTO_LEVELS = True).
Each frame dynamically adjusts intensity limits based on signal
distribution, which:
  • Enhances contrast for motion
  • Suppresses noise floor variations
  • Improves visual clarity for analysis
As a result, the background may change dynamically instead of
remaining fixed deep-blue as in earlier versions.
This affects display ONLY — saved tensors for CNN training
remain unchanged.
To restore fixed blue background:
    Set AUTO_LEVELS = False and use fixed dB limits
...
RADAR_CFG_FILE = r".\config\awr2944_ASL_2.cfg"  #r".\awr2944_cfg_updated.cfg"
...
ANGLE_UPDATE_EVERY = 4      # compute RA every N frames Note: changed from 2
...
def parse_awr_cfg(cfg_text: str) -> RadarFrameConfig:
    ...
    if None in (adc_samples, loops, rx_channels, tx_channels):
        raise ValueError("Could not parse config correctly.")

    return RadarFrameConfig(
...
class RadarUI(QtWidgets.QMainWindow):
    def __init__(self, cfg: RadarFrameConfig):
        ...
        self.dsp = DSPState(cfg.loops, cfg.adc_samples)
